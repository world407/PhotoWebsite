...
// Community component, not part of DavidHDev/react-bits main repo.
// Ported to TypeScript for PhotoWebsite with reduced-motion fallback
// and off-viewport animation pausing.
...
export interface OrbitImage {
  /** 图片 URL */
  src: string;
  /** 替代文本；装饰性场景传空字符串（容器已 aria-hidden） */
  alt?: string;
}
...
function generateInfinityPath(cx: number, cy: number, w: number, h: number): string {
  const hw = w / 2;
  const hh = h / 2;
  return `M ${cx} ${cy} C ${cx + hw * 0.5} ${cy - hh}, ${cx + hw} ${cy - hh}, ${cx + hw} ${cy} C ${cx + hw} ${cy + hh}, ${cx + hw * 0.5} ${cy + hh}, ${cx} ${cy} C ${cx - hw * 0.5} ${cy + hh}, ${cx - hw} ${cy + hh}, ${cx} ${cy} C ${cx - hw * 0.5} ${cy - hh}, ${cx - hw} ${cy - hh}, ${cx} ${cy}`;
}
...
export function OrbitImages({
  ...
  const animationActive = useAnimationActive(containerRef);
  const effectivePaused = paused || reducedMotion || !animationActive;
  ...
  }, [progress, duration, easing, direction, effectivePaused]);
